...
import common.{Dict, File, WordCount, otocLogg}
import org.apache.spark.rdd.RDD
import org.apache.spark.sql.{Dataset, SparkSession}

object DocsSourceData {

  private def dsDoc(spark: SparkSession, url_string: String, docId: Int): Dataset[File] = {
  ...
  def getDatasetDict(spark: SparkSession, url_string: String): Dataset[Dict] = {
    import spark.implicits._
    val ds= spark
      .read.format("jdbc")
      .option("url", url_string)
  ...
  def getListOfUncalDocs(spark: SparkSession, url_string: String): List[Int] = {
    import spark.implicits._
    val ds = spark
      .read.format("jdbc")
      .option("url", url_string)
  ...
  def getWordCountByDocId(spark: SparkSession, url_string: String, docId: Int): Dataset[WordCount] ={
    import spark.implicits._
    val srcDataSet: Dataset[File] = dsDoc(spark,url_string,docId)
